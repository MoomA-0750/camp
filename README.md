...
**Phase 5（編集面）完了、2026-09-14。** Obsidian と Camp の両方から書く運用で、Obsidian からは画面を作り込む段で徐々に移す。 Phase 4（ビュー機構の移行・時系列チャート・グラフビュー、M50〜M52）は 2026-09-13 に完了して本番へ反映し、`.base` との併読期間に入った。
...
| 4 | ビュー機構の移行（`.base` から独自定義へ）・チャート・グラフビュー | **完了**（M50〜M52、2026-09-13。本番反映済み、`.base` と併読中） |
| 5 | 編集面（CodeMirror 6） | **完了**（M53〜M55・outer gate、2026-09-14。M53・M54 は本番反映済み。書き手の移行は外し、画面を作り込む段で両方から書きながら徐々に移す） |
| 6 | モバイル（まずPWA） | 未着手 |
| 将来 | どの範囲まで動けるかの選択（全エージェント同時に） | 後で必ずやる |

**Phase 5 から Camp は Vault に書く**（書くのは本人のユーザーで動く実行面。campd は書けない）。書いたノートは自動で commit・push する。ノートを開く（Ctrl+O）・wikilink の補完・新しいノート・日次ログ・`.trash/` へ移すもある（M55）。当面は Obsidian と Camp の両方から書く。Obsidian から移すのは、画面を作り込む段で両方を触りながら徐々に（本人の方針、2026-09-14）。
...
**索引は Vault に書かない**（書くのは Phase 5 の編集面だけで、実行面が書く）。索引は 4,170ファイル（markdown 4,121）・15.4MiB、走査18ms・索引2.4秒（2回目645ms）。ドット始まりのディレクトリは降りる前に切る（`.claude/worktrees/` にこの Vault の11倍のファイルがある）。
...
- 実行面は、**手元で起こせるもの**と、**手元に実体は無いが向こうのホストでなら起こせるもの**を
  名乗る（2026-09-13）。後者には `remote_only` が付き、画面では「このマシン」を選べなくなる。
  Codex を手元で起こせるのは `codex` が見つかり scope で包めるときだけ（`campd agent -codex <path>`。
  本人の置き場は `$CODEX_HOME` か `~/.codex`）だが、**手元に無くても向こうのホストでは頼める**
  （向こうで探す実体は台帳の場所か向こうの PATH）。Claude も同じ扱い——手元に `claude` が
  無くても実行面は起こせる
