...
**Phase 3.7 進行中（CLI の忠実なリモコンに戻し、エージェントを対等にする）。** M39（Codex を本人の置き場で）・M40（エージェントごとの違いを駆動器の形に）・M41（確認の度合いをセッションごとに）・M42（向こうのホストでも全エージェント）まで済み（2026-09-12）。Phase 0〜3、Phase 2.5（保持設計、M21〜M25）、M25.5（権限境界）、Phase 3.5（SSH経由のリモート起動）、Phase 3.6（Codex のセッション駆動）は済み。2026-09-01にリポジトリ作成。

次は **M43（文書と outer gate: 本物の Claude Code・Codex をこのマシンと `rp` で）**。
...
| 3.7 | CLI の忠実なリモコンに戻す・エージェントを対等に（本人の置き場・駆動器の形・セッションごとの確認の度合い・向こうのホストでも全エージェント） | 進行中（M39〜M42 済み） |
| 3.8 | 記録の対等（Codex・向こうの会話記録の取り込み、Codex のプラン枠の履歴） | 未着手 |
| 4 | ビュー機構の移行（`.base` から独自定義へ）・チャート・グラフビュー | 未着手 |
| 5 | 編集面（CodeMirror 6）→ 書き手の移行 | 未着手 |
| 6 | モバイル（まずPWA） | 未着手 |
| 将来 | 終わったセッションの再開・どの範囲まで動けるかの選択（どちらも全エージェント同時に） | 後で必ずやる |
...
- **放置やターンの長さで Camp からは止めない**（D-030、2026-09-12）。CLI と同じく、終わるのは本人が
  ...
  承認だけは 4分30秒で拒否する（Claude 自身のパーク期限 5 分に合わせたもの）。同時に走らせられるのは
  4本までなので、使い終わったセッションは止める
...
- **コマンドとファイル変更以外の要求には、いまは自動で断りを返す**（質問・権限を広げる願い・MCP の
  問い合わせ。CLI なら本人に訊くところ）。画面から答えられるようにするのは Phase 3.8（本人の決定、
  2026-09-12）
...
- Codex の会話記録は取り込まない（Phase 3.8）。画面に出るのは実行面が受け取ったフレーム
- エージェントごとの違いは駆動器に閉じてあり、セッションの画面・API は駆動器の説明だけを見る
  （docs/30-session-protocol.md の 12 節）。ただし実行面の設定の口（`campd agent -claude`・`-codex`、
  systemd の環境変数）・古い実行面との互換（名乗らなければ Claude とみなす）・会話記録の取り込みと
  「残量」の画面（Claude だけ。Codex は Phase 3.8）には、まだエージェントの名前がある
