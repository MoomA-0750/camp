...
| 口 | 欄 |
|---|---|
| hello | `agents`（起こせる名前の配列。古い campd のために残す）と `drivers`（駆動器の説明: `name`・`label`・`perms`・`notes`・`interrupt_leaves_tools`・`remote`・`remote_only`・`resume`）。**名乗らない古い実行面**の分は campd が手元の駆動器から補い、確認の度合いは `cli` だけとみなす |
| hello | `remote_only`（M49、2026-09-13）: **この実行面では手元に実体が無いが、向こうのホストでなら起こせる**。名乗りには載るが、このマシンでは起こさない。**欄が無いことを「手元でも起こせる」と読む**——逆向き（`local`）にすると、この欄を知らない実行面の名乗りが「手元では起こせない」と読まれ、いままで起こせていたものが起こせなくなる |
